...
Chạy lệnh từ root repository với Python >= 3.10. Trên Ubuntu/WSL, dùng môi
trường ảo để tránh lỗi `externally-managed-environment` của Python hệ thống:
...
python eng.py knowledge organize --file knowledge/inbox/my-note.md --dry-run
python eng.py knowledge organize --file knowledge/inbox/my-note.md
...
Install the declared PDF dependencies (`pypdf` and `fonttools`) through the
project virtual environment before PDF ingestion or the full test suite.

`add-knowledge` accepts exactly one file, direct text, or stdin source and saves
it under `knowledge/inbox/`. It accepts UTF-8 `.md`, `.markdown`, and `.txt`, as
well as text-based `.pdf` files. It copies external input, never overwrites,
deduplicates unchanged content, and indexes automatically. If indexing fails,
the saved document remains and the command prints an exact retry command while
returning nonzero. `--no-index` intentionally skips indexing and does not claim
RAG readiness.

The canonical [Knowledge Ingestion and Hybrid RAG Guide](KNOWLEDGE_INGESTION.md)
documents web equivalents, exact states, duplicate/collision behavior, the full
path to RAG, filter/debug/log behavior, and partial-failure recovery.
