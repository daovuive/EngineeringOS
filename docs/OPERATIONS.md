...
python eng.py knowledge search "REQ-SDV-0012" --debug
python eng.py knowledge ask "What is the decision?" --project eos
```

Install the declared PDF dependencies (`pypdf` and `fonttools`) through the
project virtual environment before PDF ingestion or the full test suite. The complete
import/rebuild/filter/debug/log recovery workflow is canonical in
[Knowledge Ingestion and Hybrid RAG](KNOWLEDGE_INGESTION.md). Logs remain off by
default; enabling allowlisted logs changes the explicit rebuild input set.
